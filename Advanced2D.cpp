...
namespace GAM200
{
	Engine::Engine()
	{
		...
		//set default values
		this->setAppTitle("WHATISMINE");
		this->setScreenWidth(GetSystemMetrics(SM_CXSCREEN));
		this->setScreenHeight(GetSystemMetrics(SM_CYSCREEN));
	...
	{
		std::ostringstream s;
		s << "Splice Engine v" << p_versionMajor << "." << p_versionMinor << "." << p_revision;
		return s.str();
	}
	...
	void Engine::Update()
	{
		...
		//perform global collision testing at 20hz
	...
	void Engine::UpdateEntities()
	{
		...
		{
			//pointer to local sprite to object in the list
			entity = *iter;
      ...
      if ((*first)->getRenderType() == RENDER2D)
      {
        ...
        if (sprite1->getAlive() && sprite1->getVisible() && sprite1->isCollidable())
        {
          ...
          while (second != p_entities.end())
          {
            ...
            if (sprite2->getAlive() && sprite2->getVisible() && sprite2->isCollidable() && sprite2 != sprite1)
            {
              //test for collision
  ...
  bool Engine::collisionBR(Sprite *sprite1, Sprite *sprite2)
  {
    ...
    Rect *ra = new Rect(
      sprite1->getX(),
      sprite1->getY(),
      sprite2->getX() + sprite1->getWidth()*sprite1->getScale(),
      sprite2->getY() + sprite1->getHeight()*sprite1->getScale());
